...
const supabaseUrl = 'https://wotvfhwiqsgkgqcrwupo.supabase.co'
const supabaseAnonKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6IndvdHZmaHdpcXNna2dxY3J3dXBvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTEyMDQ2MDksImV4cCI6MjA2Njc4MDYwOX0.fXz71KhcdhYXj8rhM7ehogU2AmZFh7T8wJ9t1-dXFMk'
...
export class SupabaseService {
  ...
  async getWeightConfigurations(): Promise<WeightConfig[]> {
    ...
    if (error) {
      ...
      throw new Error('Failed to fetch weight configurations')
    }
  ...
  private mapEmployeeFromDB(data: any): Employee {
    try {
      ...
      const employee: Employee = {
        ...
        dataAdmissao: new Date(data.data_admissao || Date.now()),
        dataUltimaPromocao: new Date(data.data_ultima_promocao || Date.now()),
        ...
        tempoDeCasa: parseFloat(data.tempo_de_casa) || 0,
        tempoNoCargo: parseFloat(data.tempo_no_cargo) || 0,
        ...
        tempoUltimaPromocao: this.calculateYearsDiff(new Date(data.data_ultima_promocao || Date.now()), new Date()),
